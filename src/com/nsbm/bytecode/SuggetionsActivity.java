...
public class SuggetionsActivity extends Activity {
	...
	private void addAlreadyReadList(Book book) {
		...
		Uri uri = getContentResolver().insert(AlreadyReadEntry.CONTENT_URI, values);
		Log.d("SuggestionActivity",ContentUris.parseId(uri)+"Lol");
	}

	private void addWantsToReadList(Book book) {
		ContentValues values = BookUtilities.getContentValues(book);
		Uri uri = getContentResolver().insert(WantsToReadEntry.CONTENT_URI, values);
		Log.d("SuggestionActivity Wants",ContentUris.parseId(uri)+"Lol");
